...
# Successes
Our group was lucky in that our availability matched up well, and successful in getting a head start so that we were not in a time crunch at all. We were able to meet early in the week and discuss the psuedocode in detail, which made the implementation very straightforward. Because we were able to go over the algorithm pretty thouroughly, we all felt comfident going into coding it. 
